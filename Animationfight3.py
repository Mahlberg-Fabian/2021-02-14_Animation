...
import pygame
from pygame.constants import (QUIT, K_KP_PLUS, K_KP_MINUS, K_ESCAPE, KEYDOWN, K_SPACE)
import os
...
class Cat(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.images = []
        for i in range(8):                          # Animations-Sprites laden §\label{srcAnimation0001}§
            bitmap = pygame.image.load(Settings.imagepath(f"fight3{i}.png")).convert()
            bitmap.set_colorkey((0,0,0))
            self.images.append(bitmap)
        self.imageindex = 0                         # Image 1 blitten
        self.image = self.images[self.imageindex]
        self.rect = self.image.get_rect()
        self.rect.center = (Settings.window['width'] // 2, Settings.window['height'] // 2)       # Zentrum der Katze ist Zentrum des Bitmaps
        self.animation_time = Timer(100)       # Timer auf 100 Millisekunden (1 Sekunde)

    def update(self):
        if self.animation_time == Timer(100):
            stop
...
class CatAnimation(object):            # Hauptprogramm
